...
private const val TAG = "LiveStoryPagingSource"
class LiveStoryPagingSource constructor(
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, BookItem> {
        return try {
            ...
            val response = remoteDataSources.getLiveStoryList(
                page = page,
                userID = userID
            )
            LoadResult.Page(
                data = response.liveStoriesList,
                prevKey = if (page == 1) null else page - 1,
                nextKey = if (response.last_page == page) null else page + 1
            )
        } catch (e: Exception) {
            MyCustomLogger.logInfo(tag = TAG, message = "error response ${e.message}")
            LoadResult.Error(e)
        }
    }
}